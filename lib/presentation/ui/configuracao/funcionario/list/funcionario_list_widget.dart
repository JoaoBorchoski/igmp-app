...
class FuncionarioListWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return AppDismissible(direction: authentication.permitUpdateDelete('/funcionarios'),
      endToStart: () async {
        await Provider.of<FuncionarioRepository>(context, listen: false).delete(funcionario).then((message) {
      ...
      body: Column(
        children: <Widget>[
          Card(
            ...
            elevation: 5,
            child: ListTile(title: Text((funcionario.nome ?? '').toString(),
                  style: TextStyle(color: AppColors.cardTextColor,)),
              subtitle: Text((funcionario.cpf ?? '').toString(),
                  style: TextStyle(color: AppColors.cardTextColor,)),
            ),
          ),
        ],
