...
class _FuncionarioListPageState extends State<FuncionarioListPage> {
  ...
  Future<void> _fetchData() async {
    ...
    await Provider.of<FuncionarioRepository>(context, listen: false).list(_query, _pageSize, _page, ['ASC', 'ASC']).then((value) {
  ...
  void _hasErrorDialog() async {
    await showDialog(
            context: context,
            builder: (context) => ConfirmActionWidget(message: 'Ocorreu um erro ao carregar as funcionarios.', cancelButtonText: 'Tentar novamente'))
        .then((value) {
      setState(() {
        _isLoading = true;
        _hasError = false;
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(onWillPop: () async {
      ...
      child: AppScaffold(title: Text('Funcionarios'),
