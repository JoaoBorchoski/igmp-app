...
class AlizarFormPage extends StatefulWidget {
  const AlizarFormPage({super.key});

  @override
  State<AlizarFormPage> createState() => _AlizarFormPageState();
}

class _AlizarFormPageState extends State<AlizarFormPage> {
  final _formKey = GlobalKey<FormState>();
  bool _dataIsLoaded = false;
  bool _isViewPage = false;

  final _controllers = AlizarController(
      ...
      onWillPop: () async {
        ...
        _isViewPage
            ? Navigator.of(context)
                .pushNamedAndRemoveUntil('/alizares', (route) => false)
            : await showDialog(
                context: context,
                builder: (context) {
  ...
  Future<void> _loadData(String id) async {
    await Provider.of<AlizarRepository>(context, listen: false)
        .get(id)
        .then((alizar) => _populateController(alizar));
  }

  Future<void> _populateController(Alizar alizar) async {
    setState(() {
      _controllers.id!.text = alizar.id ?? '';
      _controllers.nome!.text = alizar.nome ?? '';
      _controllers.descricao!.text = alizar.descricao ?? '';
    });
  }
        ...
        if (validado) {
          return showDialog(
            ...
            builder: (context) {
              ...
              Navigator.of(context).pushReplacementNamed('/alizares'));
        }
      });
