...
class SentidoAberturaFormPage extends StatefulWidget {
  const SentidoAberturaFormPage({super.key});

  @override
  State<SentidoAberturaFormPage> createState() =>
      _SentidoAberturaFormPageState();
}

class _SentidoAberturaFormPageState extends State<SentidoAberturaFormPage> {
  final _formKey = GlobalKey<FormState>();
  bool _dataIsLoaded = false;
  bool _isViewPage = false;

  final _controllers = SentidoAberturaController(
      ...
      onWillPop: () async {
        ...
        _isViewPage
            ? Navigator.of(context)
                .pushNamedAndRemoveUntil('/sentidos-abertura', (route) => false)
            : await showDialog(
                context: context,
                builder: (context) {
                ...
                ? Navigator.of(context).pushNamedAndRemoveUntil(
                    '/sentidos-abertura', (route) => false)
                : retorno = value);

        return retorno;
      },
      child: AppScaffold(
        title: Text('SentidosAbertura Form'),
        showDrawer: false,
  ...
  Future<void> _loadData(String id) async {
    await Provider.of<SentidoAberturaRepository>(context, listen: false)
        .get(id)
        .then((sentidoAbertura) => _populateController(sentidoAbertura));
  }

  Future<void> _populateController(SentidoAbertura sentidoAbertura) async {
    setState(() {
      _controllers.id!.text = sentidoAbertura.id ?? '';
      _controllers.nome!.text = sentidoAbertura.nome ?? '';
      _controllers.descricao!.text = sentidoAbertura.descricao ?? '';
    });
  }
        ...
        if (validado) {
          return showDialog(
            ...
            builder: (context) {
              ...
              Navigator.of(context).pushReplacementNamed('/sentidos-abertura'));
        }
      });
