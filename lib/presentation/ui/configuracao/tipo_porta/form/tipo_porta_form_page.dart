...
class TipoPortaFormPage extends StatefulWidget {
  const TipoPortaFormPage({super.key});

  @override
  State<TipoPortaFormPage> createState() => _TipoPortaFormPageState();
}

class _TipoPortaFormPageState extends State<TipoPortaFormPage> {
  final _formKey = GlobalKey<FormState>();
  bool _dataIsLoaded = false;
  bool _isViewPage = false;

  final _controllers = TipoPortaController(
      ...
      onWillPop: () async {
        ...
        _isViewPage
            ? Navigator.of(context)
                .pushNamedAndRemoveUntil('/tipos-porta', (route) => false)
            : await showDialog(
                context: context,
                builder: (context) {
  ...
  Future<void> _loadData(String id) async {
    await Provider.of<TipoPortaRepository>(context, listen: false)
        .get(id)
        .then((tipoPorta) => _populateController(tipoPorta));
  }

  Future<void> _populateController(TipoPorta tipoPorta) async {
    setState(() {
      _controllers.id!.text = tipoPorta.id ?? '';
      _controllers.nome!.text = tipoPorta.nome ?? '';
      _controllers.descricao!.text = tipoPorta.descricao ?? '';
    });
  }
        ...
        if (validado) {
          return showDialog(
            ...
            builder: (context) {
              ...
              Navigator.of(context).pushReplacementNamed('/tipos-porta'));
        }
      });
