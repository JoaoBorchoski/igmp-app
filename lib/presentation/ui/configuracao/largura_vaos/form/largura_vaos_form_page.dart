...
class LarguraVaosFormPage extends StatefulWidget {
  const LarguraVaosFormPage({super.key});

  @override
  State<LarguraVaosFormPage> createState() => _LarguraVaosFormPageState();
}

class _LarguraVaosFormPageState extends State<LarguraVaosFormPage> {
  final _formKey = GlobalKey<FormState>();
  bool _dataIsLoaded = false;
  bool _isViewPage = false;

  final _controllers = LarguraVaosController(
      ...
      onWillPop: () async {
        ...
        _isViewPage
            ? Navigator.of(context)
                .pushNamedAndRemoveUntil('/larguras-vaos', (route) => false)
            : await showDialog(
                context: context,
                builder: (context) {
  ...
  Future<void> _loadData(String id) async {
    await Provider.of<LarguraVaosRepository>(context, listen: false)
        .get(id)
        .then((larguraVaos) => _populateController(larguraVaos));
  }

  Future<void> _populateController(LarguraVaos larguraVaos) async {
    setState(() {
      _controllers.id!.text = larguraVaos.id ?? '';
      _controllers.nome!.text = larguraVaos.nome ?? '';
      _controllers.descricao!.text = larguraVaos.descricao ?? '';
    });
  }
        ...
        if (validado) {
          return showDialog(
            ...
            builder: (context) {
              ...
              Navigator.of(context).pushReplacementNamed('/larguras-vaos'));
        }
      });
