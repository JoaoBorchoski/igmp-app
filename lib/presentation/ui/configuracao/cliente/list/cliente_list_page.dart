...
class _ClienteListPageState extends State<ClienteListPage> {
  ...
  Future<void> _fetchData() async {
    ...
    await Provider.of<ClienteRepository>(context, listen: false).list(_query, _pageSize, _page, ['ASC', 'ASC']).then((value) {
        clienteList = value;
        setState(() {
  ...
  void _hasErrorDialog() async {
    await showDialog(
            context: context,
            builder: (context) => ConfirmActionWidget(message: 'Ocorreu um erro ao carregar as clientes.', cancelButtonText: 'Tentar novamente'))
        .then((value) {
      setState(() {
        _isLoading = true;
        _hasError = false;
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(onWillPop: () async {
      ...
      child: AppScaffold(title: Text('Clientes'),
        route: '/clientes-form',
        showDrawer: true,
        body: Center(
          child: Column(
