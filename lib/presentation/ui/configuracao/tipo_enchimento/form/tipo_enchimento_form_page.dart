...
class TipoEnchimentoFormPage extends StatefulWidget {
  const TipoEnchimentoFormPage({super.key});

  @override
  State<TipoEnchimentoFormPage> createState() => _TipoEnchimentoFormPageState();
}

class _TipoEnchimentoFormPageState extends State<TipoEnchimentoFormPage> {
  final _formKey = GlobalKey<FormState>();
  bool _dataIsLoaded = false;
  bool _isViewPage = false;

  final _controllers = TipoEnchimentoController(
      ...
      onWillPop: () async {
        ...
        _isViewPage
            ? Navigator.of(context)
                .pushNamedAndRemoveUntil('/tipos-enchimento', (route) => false)
            : await showDialog(
                context: context,
                builder: (context) {
                ...
                ? Navigator.of(context).pushNamedAndRemoveUntil(
                    '/tipos-enchimento', (route) => false)
                : retorno = value);

        return retorno;
      },
      child: AppScaffold(
        title: Text('TiposEnchimento Form'),
        showDrawer: false,
  ...
  Future<void> _loadData(String id) async {
    await Provider.of<TipoEnchimentoRepository>(context, listen: false)
        .get(id)
        .then((tipoEnchimento) => _populateController(tipoEnchimento));
  }

  Future<void> _populateController(TipoEnchimento tipoEnchimento) async {
    setState(() {
      _controllers.id!.text = tipoEnchimento.id ?? '';
      _controllers.nome!.text = tipoEnchimento.nome ?? '';
      _controllers.descricao!.text = tipoEnchimento.descricao ?? '';
    });
  }
        ...
        if (validado) {
          return showDialog(
            ...
            builder: (context) {
              ...
              Navigator.of(context).pushReplacementNamed('/tipos-enchimento'));
        }
      });
