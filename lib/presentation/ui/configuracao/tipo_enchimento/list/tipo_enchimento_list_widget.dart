...
class TipoEnchimentoListWidget extends StatelessWidget {
  final TipoEnchimento tipoEnchimento;

  const TipoEnchimentoListWidget(
    this.tipoEnchimento, {
    Key? key,
  }) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return AppDismissible(direction: authentication.permitUpdateDelete('/tipos-enchimento'),
      endToStart: () async {
        await Provider.of<TipoEnchimentoRepository>(context, listen: false).delete(tipoEnchimento).then((message) {
      ...
      startToEnd: () {
        Map data = {'id': tipoEnchimento.id, 'view': false};
        Navigator.of(context).pushReplacementNamed('/tipos-enchimento-form', arguments: data);
      },
      onDoubleTap: () {
        Map data = {'id': tipoEnchimento.id, 'view': true};
        Navigator.of(context).pushReplacementNamed('/tipos-enchimento-form', arguments: data);
      },
      body: Column(
        children: <Widget>[
          Card(
            ...
            elevation: 5,
            child: ListTile(title: Text((tipoEnchimento.nome ?? '').toString(),
                  style: TextStyle(color: AppColors.cardTextColor,)),
              subtitle: Text((tipoEnchimento.descricao ?? '').toString(),
                  style: TextStyle(color: AppColors.cardTextColor,)),
            ),
          ),
        ],
