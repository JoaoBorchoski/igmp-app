...
class AlturaVaosFormPage extends StatefulWidget {
  const AlturaVaosFormPage({super.key});

  @override
  State<AlturaVaosFormPage> createState() => _AlturaVaosFormPageState();
}

class _AlturaVaosFormPageState extends State<AlturaVaosFormPage> {
  final _formKey = GlobalKey<FormState>();
  bool _dataIsLoaded = false;
  bool _isViewPage = false;

  final _controllers = AlturaVaosController(
      ...
      onWillPop: () async {
        ...
        _isViewPage
            ? Navigator.of(context)
                .pushNamedAndRemoveUntil('/alturas-vaos', (route) => false)
            : await showDialog(
                context: context,
                builder: (context) {
  ...
  Future<void> _loadData(String id) async {
    await Provider.of<AlturaVaosRepository>(context, listen: false)
        .get(id)
        .then((alturaVaos) => _populateController(alturaVaos));
  }

  Future<void> _populateController(AlturaVaos alturaVaos) async {
    setState(() {
      _controllers.id!.text = alturaVaos.id ?? '';
      _controllers.nome!.text = alturaVaos.nome ?? '';
      _controllers.descricao!.text = alturaVaos.descricao ?? '';
    });
  }
        ...
        if (validado) {
          return showDialog(
            ...
            builder: (context) {
              ...
              Navigator.of(context).pushReplacementNamed('/alturas-vaos'));
        }
      });
