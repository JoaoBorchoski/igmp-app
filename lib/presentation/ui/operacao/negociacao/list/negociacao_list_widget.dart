...
class NegociacaoListWidget extends StatelessWidget {
  final Negociacao negociacao;

  const NegociacaoListWidget(
    this.negociacao, {
    Key? key,
  }) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return AppDismissible(
      direction: authentication.permitUpdateDelete('/negociacoes'),
      endToStart: () async {
      ...
      startToEnd: () {
        Map data = {'id': negociacao.id, 'view': false};
        Navigator.of(context)
            .pushReplacementNamed('/negociacoes-form', arguments: data);
      },
      onDoubleTap: () {
        Map data = {'id': negociacao.id, 'view': true};
        Navigator.of(context)
            .pushReplacementNamed('/negociacoes-form', arguments: data);
      },
      body: Column(
        children: <Widget>[
          Card(
            ...
            elevation: 5,
            child: ListTile(
              title: Text((negociacao.descricao ?? '').toString(),
                  style: TextStyle(
                    color: AppColors.cardTextColor,
                  )),
              subtitle: Text((negociacao.descricao ?? '').toString(),
                  style: TextStyle(
                    color: AppColors.cardTextColor,
                  )),
            ),
